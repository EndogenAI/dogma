...
governed an architectural decision: unconditionally, before the work begins. Empirical operationalization: [external-team-case-study.md](../pm/external-team-case-study.md) § Pattern 1 (Protocol-First Adoption Ladder) demonstrates that template sufficiency — completing the fill-in-the-blank encoding checkpoint without theoretical understanding of H1–H3 — achieved 100% compliance across 20 sessions, confirming template engagement as the correct operationalization mechanism for H4 adoption, not theory transfer.

**Programmatic Governance Stack (H1 × H4 × C4)**: Behavioral constraints are encoded at multiple enforcement tiers (T1–T5), with higher tiers providing stronger assurance. Prompt-level constraints (T1–T2) state the values; script-level gates (T3) check structural compliance; pre-commit hooks (T4) prevent non-compliant commits; runtime sandboxing (T5) enforces boundaries at execution time. The endogenic implementation instantiates this pattern: MANIFESTO.md (T1 specification), AGENTS.md (T2 re-encoding), `validate_synthesis.py` (T3 structure check), pre-commit hooks (T4 push gate), and `.github/workflows/` CI validation (T5 integration testing). Corpus reference: [shifting-constraints-from-tokens.md](shifting-constraints-from-tokens.md) formalizes the enforcement tier hierarchy, demonstrating why programmatic encoding is superior to token-based guidance for high-assurance constraints. The Local-Compute-First axiom is a structural property of this governance architecture: enforcement scripts run locally because oversight proximity — keeping validation co-deployed with the system being governed — is a design integrity requirement, independent of cost. Corpus references: [`lcf-oversight-infrastructure.md`](../infrastructure/lcf-oversight-infrastructure.md) establishes the structural-enabler framing; [`lcf-programmatic-enforcement.md`](../infrastructure/lcf-programmatic-enforcement.md) documents the F4 observable-proxy gate design (semantic-intent: intentional human-judgment gate; observable-proxy: WARN-only `scripts/check_model_usage.py`). [workflow-formula-encoding-dsl.md](workflow-formula-encoding-dsl.md) § Case Study 1 extends Algorithms-Before-Tokens to the workflow-specification layer: EBNF DSL encoding of decision logic with 81% semantic compression, deterministic round-trip validation, and ISO/IEC 14977 grammar — the most quantitatively precise Algorithms-Before-Tokens demonstration in the corpus; pending promotion from Draft status.
...
- **H4 → H1**: The fifty-year lineage (Knuth → Nygard → Martraire → AGENTS.md) identifies *what* encode-before-act is; [agent-fleet-design-patterns.md](../agents/agent-fleet-design-patterns.md) operationalizes *when* and *how* it is invoked at session scope.

- **H1 → H3**: The encode-before-act gate ensures agents receive the substrate specification before acting, making substrate co-authorship possible. Corpus reference: [agent-fleet-design-patterns.md](../agents/agent-fleet-design-patterns.md) §H1.

- **H3 → H2**: When agents co-author the substrate, system dynamics shift: substrate changes become selection pressures on subsequent behavior. Corpus reference: [dogma-neuroplasticity.md](../neuroscience/dogma-neuroplasticity.md) §H2 Back-Propagation Protocol formalizes this feedback loop.
...
in agent fleet co-evolution within EndogenAI. External team validation remains future work: applying the full methodology to a greenfield scripting or documentation project would provide independent evidence of breadth and allow measurement of adoption barriers. The operational implementation is real; the evidence for breadth across three independent domains is partially theoretical pending external replication. The strongest available internal-proxy evidence is documented in [external-team-case-study.md](../pm/external-team-case-study.md) § Metrics M1–M5: M2=100% post-protocol compliance across 20 sessions; M4=33/33 phase-gate adoption; ARM=5 in two independent sprint events; CONTRIBUTING.md=1,066 words — distinguishing "no evidence" from "strong internal proxies, one critical external-cold-start gap."
...
**LCF F4 programmatic enforcement (partially addressed)**: The Local-Compute-First axiom's F4 gap — no CI-enforced programmatic gate — is now partially resolved. The enforcement surface is split into two independent dimensions: the semantic-intent surface (whether an agent meaningfully defers to local compute for high-stakes operations) is assigned to the intentional human-judgment tier; the observable-proxy surface (string-detectable markers of cloud-model API calls) is addressed by a recommended WARN-only gate, deferred to issue #131 for signal-to-noise calibration before FAIL-blocking is enabled. See [`lcf-programmatic-enforcement.md`](../infrastructure/lcf-programmatic-enforcement.md) and [`lcf-oversight-infrastructure.md`](../infrastructure/lcf-oversight-infrastructure.md).

### 5.3 Security Threat Model and Governance Gaps

**Threat Surface Acknowledgment**: Agent-driven workflows introduce new threat surfaces — fetch-source SSRF risk, prompt injection via externally-sourced content, and credential exposure through session context — that are orthogonal to the methodology's design contribution but critical for safe deployment. These are operational security concerns, not methodological flaws, but must be explicitly acknowledged as governance requirements. Corpus reference: [security-threat-model.md](../infrastructure/security-threat-model.md) provides a formal threat catalog and mitigation strategies for agent-driven system threats. Any deployment of the endogenic methodology should incorporate the security framework as a complementary governance layer.
...
1. What is the measured effect of encode-before-act on token efficiency and session coherence
   versus a reactive context reconstruction baseline? This measurement is currently blocked by
   a 4-step prerequisite chain: LCF baseline (issue #131) → local compute stack → episodic
   memory integration → cross-session drift detection (`validate_session.py --drift-check`).
   See [episodic-memory-agents.md](../agents/episodic-memory-agents.md) § R1 for the closed-with-wait
   verdict on episodic memory integration (issue #13) and the complete dependency map as an
   explicit open blocker with a clear unlock path.
...
- **Deployment-Layer Extension**: The five-layer encoding chain assumes single-principal ownership. Multi-principal scenarios (external team adoption, product deployment) require extension to six layers with explicit conflict-resolution rules. [external-value-architecture.md](external-value-architecture.md) provides the formal architecture specification and Supremacy constraints; [external-values-decision-framework.md](external-values-decision-framework.md) § Four-Type Conflict Taxonomy and Decision Tree formalizes the conflict-resolution logic (Type 1 Axiomatic Override, Type 2 Session Injection, Type 3 Ethical Conflict, Type 4 Provenance Suppression; ALLOW/BLOCK/ESCALATE outcome tree) — currently T5 prose-only governance, making programmatic enforcement (adopt-wizard + `resolve_values_conflict.py`) the outstanding implementation gap.
...
- **Security Threat Model**: Agent-driven workflows introduce new threat surfaces (fetch-source SSRF, prompt injection, session context credential exposure). These are operational security concerns requiring complementary governance. [security-threat-model.md](../infrastructure/security-threat-model.md) provides formal threat catalog and mitigation strategies.

- **Morphogenetic System Design Operationalization**: H2 (morphogenetic design) is grounded in Turing's morphogenesis and autopoietic theory but lacks explicit mapping to fleet emergence mechanisms. [dogma-neuroplasticity.md](../neuroscience/dogma-neuroplasticity.md) provides the formal specification of the back-propagation cycle operationalizing the morphogenetic feedback loop.
...
- See [dogma-neuroplasticity.md](../neuroscience/dogma-neuroplasticity.md) for H2 Back-Propagation Protocol and morphogenetic operationalization
- See [shifting-constraints-from-tokens.md](shifting-constraints-from-tokens.md) for enforcement-tier stack formalization
- See [security-threat-model.md](../infrastructure/security-threat-model.md) for comprehensive threat catalog and mitigation strategies
- See [lcf-oversight-infrastructure.md](../infrastructure/lcf-oversight-infrastructure.md) for LCF as structural-enabler governance property — enforcement proximity as design rationale
...
- See [lcf-programmatic-enforcement.md](../infrastructure/lcf-programmatic-enforcement.md) for F4 observable-proxy gate design — two enforcement surfaces (semantic-intent: human-judgment; observable-proxy: WARN-only check_model_usage.py, deferred to #131)
