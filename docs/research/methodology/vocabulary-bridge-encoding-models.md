...
- **When to use model-specific vs. bridge vocabulary (R2)**: Bridge vocabulary is most valuable in cross-cutting operational artifacts — AGENTS.md, session protocols, and research syntheses — that must speak to practitioners of both models simultaneously. This aligns with the MANIFESTO.md §Guiding Principles (Cross-Cutting) framing, which establishes that cross-cutting principles operate across domains without owning any single one. Use model-specific vocabulary when writing **within** a model's home document or when the mechanism matters (e.g., explaining that [4,1] encoding is the *source-side* preparation, not just "doing boundary specification"). Use bridge vocabulary when **crossing** between models in the same sentence or section — in cross-references, in AGENTS.md operational instructions that both models inform, and in any new research document that synthesizes across the two models. The test: if a reader of only one model needs to understand the sentence, use that model's vocabulary; if a reader of either model should understand it, use bridge vocabulary.
...
- [LCF Programmatic Enforcement](../infrastructure/lcf-programmatic-enforcement.md) — observable-proxy gate design for the Local Compute-First axiom; illustrates how bridge vocabulary applies to a concrete enforcement problem
- [LCF Oversight Infrastructure](../infrastructure/lcf-oversight-infrastructure.md) — structural-enabler framing that complements the enforcement surface; uses bridge terms to describe enabling relationships
