...
- **Impact**: Violates the [MANIFESTO.md §2](../../MANIFESTO.md#2-algorithms-before-tokens) axiom by increasing token burn and introduces maintenance debt when rule updates miss $2$ of $3$ locations.
...
| **Impact**: Breaks the [MANIFESTO.md §3](../../MANIFESTO.md#3-local-compute-first) promise of reliable local context — agents spend tokens re-reading the whole file when a specific anchor fails. |
...
- **Impact**: Increases cognitive load on the human orchestrator and confuses sub-agents about which source is the "Primary Endogenous" authority, violating [MANIFESTO.md §1](../../MANIFESTO.md#1-endogenous-first).
...
The March 2026 Audit confirms that the `dogma` repository has scaled its knowledge base ($95\%+$ coverage) but neglected its structural mechanics. The move to `x-governs:` in Phase 0 was a necessary prerequisite, but the audit surfaces a fleet-wide "tag rot" that undermines the BDI architecture. By consolidating redundant guardrails and enforcing XML compliance, we can reduce per-session token overhead by an estimated $12-15\%$ and restore the high-fidelity encoding required by the [MANIFESTO.md §2](../../MANIFESTO.md#2-algorithms-before-tokens) axiom.
