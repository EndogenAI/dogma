...
Finally, this source should be revisited alongside the `docs.letta.com` documentation (not cached) once OPEN_RESEARCH.md #1 (local compute) is resolved, to evaluate Letta's archival memory retrieval API as a candidate `scripts/` implementation for the session history query gap identified in [agentic-research-flows.md](../agents/agentic-research-flows.md).
