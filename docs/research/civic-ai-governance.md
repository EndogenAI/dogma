...
4. **Adopt a 30-Day Values Extraction Process Before AI Tool Adoption** — Implement the pattern from San Jose's Departmental AI Governance Mandate (Canonical Example 4): before any new AI tool, model, or agent capability is adopted into the dogma system, require a written values statement that answers: What outcomes must this tool optimize for? What constraints can it never violate? Who has override authority? This makes procurement questions explicit rather than implicit. The values statement becomes the acceptance criteria for the adoption Review gate, directly encoding [MANIFESTO.md § Ethical Values](../../MANIFESTO.md#ethical-values) into the decision workflow rather than post-hoc justification.

5. **Institute Cross-Domain Conflict Detection for AI Decisions** — Implement a pattern from San Jose's Cross-Departmental AI Governance Board (Canonical Example 5): when multiple AI agents or systems operate simultaneously (e.g., rate-limit gate + orchestrator + research fleet), create an explicit mechanism to detect when one agent's optimization violates another agent's stated values. In the dogma context: add a quarterly review step to session retrospectives specifically asking "Did any agent delegation this quarter produce an outcome that violated a governance constraint encoded by a different agent?" Log findings in `docs/research/` or as GitHub issues. Cross-agent conflict is invisible without deliberate cross-domain review.

6. **Create a Resident Right-to-Audit Equivalent for Agent Actions** — Implement the transparency mechanism from San Jose's community audit policy (Canonical Example 6) at the agent-fleet level: any project collaborator should be able to request an impact audit of how a specific agent delegation affected outcomes (which files were changed, what patterns were introduced, what constraints were enforced or ignored). The dogma scratchpad + git history already provides this — but it needs to be intentionally legible. Recommendation: add a `## Audit Trail` section to per-session scratchpads summarizing which agents acted, what decisions they made, and what the justification was. This operationalizes accountability as a structural mechanism rather than a voluntary documentation practice.

7. **Measure Governance Effectiveness with Three Outcome Metrics** — Adopt Mayor Mahan's three-metric framework (Canonical Example 7, Cross-References) adapted to agent fleet governance: (1) **Policy Alignment Rate** — % of agent delegations this sprint with documented acceptance criteria that were verified post-completion; (2) **Review Responsiveness** — average time between phase completion and Review agent APPROVED verdict; (3) **Constraint Correction Rate** — % of Review REQUEST CHANGES verdicts that were addressed within the same session before push. These metrics make governance performance measurable rather than assumed, consistent with [AGENTS.md § Value Fidelity Test Taxonomy](../../AGENTS.md#value-fidelity-test-taxonomy).
