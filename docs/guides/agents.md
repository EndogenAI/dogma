...
**Link path convention for agent and skill files**: Links that exit `.github/agents/` must use `../../`-relative paths (depth 2 from repo root, e.g. `../../AGENTS.md`). Links that exit `.github/skills/<name>/` must use `../../../`-relative paths (depth 3). Do NOT use `/`-rooted paths — VS Code's `prompts-diagnostics-provider` resolves them against the OS filesystem root on macOS, not the workspace root. The pre-commit hook `no-absolute-path-links-in-agent-files` enforces this: it blocks `](/` patterns in `.github/agents/` and `.github/skills/` files. Within-directory links (`./sibling.md`) remain relative. See Section 4 of the [`agent-file-authoring` skill](../../.github/skills/agent-file-authoring/SKILL.md) for the full rule and diagram.

> **Expected VS Code warnings (non-blocking)**: Two categories of IDE-only warnings will remain visible in the VS Code Problems panel after these fixes and are expected and non-blocking:
> - **38× `Attribute 'governs' is not supported`** — from Copilot Chat's `prompts-diagnostics-provider` internal schema, which does not read `yaml.schemas` from `settings.json`. The `governs:` key is correctly defined in `.vscode/agent-frontmatter.schema.json` and is semantically valid; the Copilot Chat extension simply does not expose a way to extend its validator. These warnings do not affect agent execution.
> - **8× `Unknown tool 'dogma-governance/*'`** — from Copilot Chat's static MCP tool validation when the dogma MCP server is not running. Start `uv run python -m mcp_server.dogma_server` to suppress. These warnings do not affect agent execution when the server is running.
