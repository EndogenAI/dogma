...
def count_open_issues(query: str) -> int:
    ...
    try:
        result = subprocess.run(
            f"cd /Users/conor/Sites/dogma && git log --all --oneline --grep={re.escape(Path(query).stem)} -- {query} | wc -l",
    ...
    output = {
        "correlations": {
            "crd_vs_task_velocity": {
                **corr_task,
                "interpretation": "High CRD correlates with task velocity" if corr_task["r"] > 0.3 else "Weak correlation",
            },
            "crd_vs_test_coverage": {
                **corr_test,
                "interpretation": "High CRD correlates with test coverage" if corr_test["r"] > 0.3 else "Weak correlation",
            },
            "crd_vs_citation_coherence": {
                **corr_cite,
                "interpretation": "High CRD correlates with consistent axiom citation" if corr_cite["r"] > 0.3 else "Weak correlation",
            },
        },
