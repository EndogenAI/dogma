/**
 * api.ts — MCP Dashboard API Client
 *
 ...
 *
 * Usage:
 *   import { getSnapshot, subscribeStream, isOffline } from './api';
 *
 *   // Polling
...
/** True when the sidecar is unreachable; initialised to true so the first
 *  render before any fetch completes is treated as offline. */
let _isOffline = true;
 ...
 */
export function isOffline(): boolean { return _isOffline; }
...
export async function getSnapshot(): Promise<MetricsSnapshot> {
  try {
    ...
    _isOffline = true;
    return fixtureData as MetricsSnapshot;
  }
}
 ...
 *
 * @param onData  Called for every SSE message.
 * @param onError Called when the stream errors.
 * @returns Unsubscribe function — call to close the EventSource.
 */
export function subscribeStream(
  onData: (data: MetricsSnapshot) => void,
  onError: () => void,
): () => void {
  const es = new EventSource('http://localhost:8000/api/metrics/stream');
  es.onopen = () => { _isOffline = false; };
  es.onmessage = (e: MessageEvent<string>) => { onData(JSON.parse(e.data) as MetricsSnapshot); };
  es.onerror = () => {
    _isOffline = true;
    onError();
  };
